...
!#######################################################################              
               if(nInf == 1)then
                  ...
                  !=====================================================
                  if(this%deg(j1) == this%deg(2))then
                     fls_inf = fls_inf - 1
                  endif
                  !=====================================================
                  sigma(j1) = 2
...
lant:                do
                  ...
                  !=====================================================
                  if( this%deg(j1) ==  this%deg(2) ) fls_inf = 1
                  !=====================================================
                  cycle ld
               endif
            endif
         ...
         elseif(prob <= (prob_rec+prob_inf))then
               ...
               ! Substituih o grau explicito das folhas pelo grau
               ! do sitio 2, que eh por assim dizer uma folha.
               ! No caso do grafo roda, this%deg(2) == 3 e eu os 
               ! chamo de 'raios', pq lembram o raio de uma bicicleta.
               ! Com a descricao implicita, para a dinamica nao
               ! importa se se trata de um grafo roda ou estrela.
               !########################################################
               if(prob < 1.0_dp * (stubs_inf - this%deg(2)*fls_inf)/(1.0_dp * stubs_inf))then
                  j1 = 1  
               else
sel:              do
                     j3 = gen%int(1,nInf)
                     if( this%deg(inf_List(j3)) == this%degMax )cycle sel
                     j1 = inf_List(j3)
                     exit sel
                  enddo sel
               endif
               ...
               if(sigma(j2) == 0)then
                  ...
                  inf_List(nInf) = j2
                  !=====================================================
                  if( this%deg(j2) ==  this%deg(2))then
                     fls_inf = fls_inf + 1
                  endif
                  !=====================================================
               endif
	...
	rho_medioQS_global = 1.0_dp * rho_medioQS_global / this%nodes
...
program main
   ...
   integer :: status_io
!=======================================================================   
   resultados = trim(adjustl('Rst_Grafo_Roda'))
...
!=======================================================================
   ...
   resultados = trim(adjustl(resultados))//'/alp_'//trim(adjustl(alp_char2))   
   call system('mkdir -p '//resultados)
   local = trim(adjustl(resultados))//"/"
!=======================================================================
   seed  = 967891968
...
!#######################################################################
   !====================================================================
   call rede%liga_roda(tam_rede)
   !====================================================================
   write(*,*) ""
   write(*,*) "O tamanho do grafo roda eh", rede%nodes
...
contains
      ...
      subroutine entradaArgumentos()
         ...
         !==============================================================
         lamb0 = 1.0d0/(10.0d0 * tam_rede)**0.5d0
